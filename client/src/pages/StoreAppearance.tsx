    ...
    <form onSubmit={handleSubmit((d) => saveMutation.mutate(d))} className="space-y-8 max-w-3xl">
      ...
      <Card className="bg-card border-border/60">
        <CardHeader className="pb-3">
          ...
          <CardDescription>
            Organise your store with categories shown in the sidebar, and subcategories within each.
          </CardDescription>
        </CardHeader>
        <CardContent className="space-y-4">
          {/* Add category */}
          <div className="flex gap-2">
            <Input
              placeholder="e.g. Weapons, Ranks, Kits…"
              value={newCategory}
              onChange={e => setNewCategory(e.target.value)}
              onKeyDown={e => e.key === "Enter" && (e.preventDefault(), addCategory())}
              data-testid="input-new-category"
            />
            <Button type="button" onClick={addCategory} variant="outline" className="shrink-0 gap-1.5">
                    ...
                    >
                      ...
                      {(subcategories[cat] || []).length > 0 && (
                        <span className="text-xs text-muted-foreground">
                          ({subcategories[cat].length} sub{subcategories[cat].length !== 1 ? "s" : ""})
                        </span>
                      )}
                    </button>
                    <button
                    ...
                    <div className="px-3 py-3 space-y-2 border-t border-border/40">
                      <div className="flex gap-2">
                        <Input
                          ...
                          value={newSubcat[cat] || ""}
                          onChange={e => setNewSubcat(p => ({ ...p, [cat]: e.target.value }))}
                          onKeyDown={e => e.key === "Enter" && (e.preventDefault(), addSubcat(cat))}
                          data-testid={`input-new-subcat-${cat}`}
                        />
                        <Button
                          type="button"
                          size="sm"
                          ...
                          {(subcategories[cat] || []).map(sub => (
                            ...
                            >
                              ...
                              <button
                                ...
                                data-testid={`remove-subcat-${cat}-${sub}`}
                              >
